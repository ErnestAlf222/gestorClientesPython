...
class CreateClientWindow(Toplevel,CenterWidgetMixin):
    ...
    def build(self):
        ...
        crear = Button(frame, text='Crear', command=self.createCliente)
    ...
    def validate(self, event,index):
        ...
        valido = helpers.dniValidate(valor,db.Clientes.lista) if index ==0 \
            else (valor.isalpha() and len(valor) >=2 and len(valor) <=30)
        event.widget.configure({'bg':'Green' if valido else 'Red' })

        # Cambiar el estado del bton en base a las validaciones
...
class EditClientWindow(Toplevel,CenterWidgetMixin):
    ...
    def build(self):
        ...
        actualizar = Button(frame, text='Actualizar', command=self.updateClient)
